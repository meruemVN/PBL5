...
def user_signup(request):
    if request.method == 'POST':
        ...
        # Check that the password contains at least one special character
            ...
            messages.success(request, "Đăng nhập để tiếp tục!")
...
def user_signin(request):
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(request, "Tạo tài khoản.")
            return redirect('user_signup')
        if user.check_password(password):
            ...
            if user.age != None:
                ...
                messages.success(request, "Vui lòng nhập thông tin của bạn.")
...
def collect(request):
    if request.method == 'POST':
        ...
        # Validate form data
        if not age.isdigit():
            messages.error(request, "Hãy nhập một số hợp lệ.")
            return redirect('collect')
            ...
            user.weight = weight
            user.save()
            messages.success(request, "Thông tin chi tiết đã được nhập!")
